...
func (r *Room) MoveBots() {
	...
	for _, pc := range r.playerConns {
		...
		if pc.Autopilot.On {
			...
			if Sx >= float64(arena_radius)-min_dist {
				pc.AngleRadians = a
			}
			if Sx <= -float64(arena_radius)+min_dist {
				pc.AngleRadians = a
			}
			if Sy >= float64(arena_radius)-min_dist {
				pc.AngleRadians = a
			}
			if Sy <= -float64(arena_radius)+min_dist {
				pc.AngleRadians = a
...
func (r *Room) CheckCollision(pn int64) {
	...
	for _, pc := range r.playerConns {
		...
		if x >= float64(arena_radius)-min_dist {
			crash = true
		}
		if x <= -float64(arena_radius)+min_dist {
			crash = true
		}
		if y >= float64(arena_radius)-min_dist {
			crash = true
		}
		if y <= -float64(arena_radius)+min_dist {
			crash = true
